...
--  Gồm 7 bảng: users, categories, foods,
...
CREATE TABLE foods (
    ...
    price        DECIMAL(10,0) NOT NULL,          -- VND, khong can thap phan
...
--  order_items
-- ------------------------------------------------------------
-- Don 1: Com suon x2 + Tra dao x1 = 115,000
...
-- Xem mon an theo danh muc
-- SELECT f.id, c.name AS category, f.name, f.price, f.is_available
-- FROM foods f JOIN categories c ON f.category_id = c.id
-- ORDER BY c.sort_order, f.id;

-- Xem don hang + so mon + tong tien
-- SELECT o.order_code, o.user_name, o.status, o.total_amount,
--        COUNT(oi.id) AS so_mon
-- FROM orders o JOIN order_items oi ON o.id = oi.order_id
-- GROUP BY o.id ORDER BY o.created_at DESC;

-- Xem chi tiet 1 don hang (order_id = 1)
-- SELECT oi.food_name, oi.quantity, oi.unit_price, oi.subtotal
-- FROM order_items oi WHERE oi.order_id = 1;

-- Xem thanh toan
-- SELECT payment_code, order_code, amount, payment_method, status, paid_at
-- FROM payments ORDER BY created_at DESC;

-- Xem thong bao cua user (user_id = 2)
-- SELECT type, message, is_read, created_at
-- FROM notifications WHERE user_id = 2 ORDER BY created_at DESC;


-- ============================================================
--  GHI CHU QUAN TRONG CHO TEAM
-- ============================================================
-- 1. Tat ca service deu ket noi vao CUNG 1 database:
...
--
-- 2. Moi service chi doc/ghi cac bang thuoc pham vi cua minh:
--       User Service    (8081) -> bang: users
--       Food Service    (8082) -> bang: categories, foods
--       Order Service   (8083) -> bang: orders, order_items
--       Payment Service (8084) -> bang: payments, notifications
--
-- 3. Password mau cho tat ca tai khoan: "password"
--    Hash hien tai la BCrypt cua chuoi "password".
--    Muon doi mat khau, encode lai trong Java:
--       new BCryptPasswordEncoder().encode("Password@123")
--
-- 4. Khi chay LAN nhieu may, may nao cai MySQL thi:
--    - Neu chay tren may local thi giu localhost
--    - Mo port 3306 tren firewall
--    - Cap quyen remote:
--         GRANT ALL PRIVILEGES ON food_ordering_db.* TO 'root'@'%' IDENTIFIED BY 'yourpassword';
--         FLUSH PRIVILEGES;
-- ============================================================